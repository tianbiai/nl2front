<template>
  <aside class="sidebar" :class="{ 'is-collapsed': appStore.sidebarCollapsed }">
    ...
    <div class="sidebar-header" @click="goHome">
      <div class="logo">
        <el-icon :size="26" color="var(--primary-color)"><Grid /></el-icon>
      </div>
      <transition name="fade">
        <span v-if="!appStore.sidebarCollapsed" class="logo-text">项目管理基座</span>
      </transition>
    </div>
            ...
            <transition name="slide">
              ...
              >
                ...
                >
                  ...
                  <span v-if="item.type === 'web'" class="item-badge web">WEB</span>
                  <span v-else-if="item.type === 'mobile'" class="item-badge mobile">
                    <el-icon :size="10"><Iphone /></el-icon>
                  </span>
                  <span v-else-if="item.type === 'swagger'" class="item-badge api">API</span>
                  <span v-else-if="item.type === 'doc'" class="item-badge doc">DOC</span>
                </div>
              </div>
            </transition>
          </div>
    ...
    >
      <div class="help-content">
        <div class="help-header">
          <div class="help-icon">
            <el-icon :size="32" color="var(--primary-color)"><Grid /></el-icon>
          </div>
          <h3>项目管理聚合基座</h3>
          <p>统一管理多个前端项目的聚合平台</p>
        </div>
...
function handleItemClick(item) {
  ...
  } else if (item.type === 'swagger' && item.url) {
    // swagger 类型直接跳转到 URL
    let targetUrl = item.url

    // 如果启用了 Basic 认证，构建带认证信息的 URL
    if (item.useAuth && item.authUsername && item.authPassword) {
      try {
        const url = new URL(item.url)
        url.username = item.authUsername
        url.password = item.authPassword
...
onMounted(() => {
  ...
  if (savedExpanded) {
    ...
    menuStore.groups.forEach(group => {
